...
We choose the model 6 as the best model based on the IoU score. The train and validation error are shown in \autoref{fig:train_validation_errors}. We see that there are some peaks but they decrease after the rescheduling after 50 epochs. \autoref{fig:precision_recall} shows how we set the threshold for this model.
...
\centering
    \begin{minipage}[b]{.45\columnwidth}
        ...
        \caption{Train and validation errors during training of the best model (no. 6).}
...
\begin{figure}[tbp]
    \centering
    \includegraphics[width=.8\columnwidth]{report/images/prediction.png}
    \caption{Prediction of the best model on an image from the Test set.}
    \label{fig:prediction}
\end{figure}

\section{Discussion}
We convince ourselves that this model works well by looking at the predictions it makes. \autoref{fig:prediction} shows the prediction of the model on a test image. We see that the model recognizes roofs really well and manages to exclude big obstacles. However small obstacles on the roof are harder to be recognised by the model and changing patterns on the roof can fool the model into thinking there is an obstacle. 
%TODO add discussion about the fact that moving the threshold can be better to have less FP or FN.
Adding more labelled images to the dataset would probably also make for 

\section{Conclusion}
Conclusion is done here.

\section*{Acknowledgements}
We would like to thank Pr. Castello and Simon Roquette from his lab for their guidance and helping advices during this project.
